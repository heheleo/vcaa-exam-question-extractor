...
from src.post_ocr import group_questions, extract_number, split_mcq_short_answer
...
def process_paper(
    ...
    # Clear images from a previous run so a renamed scheme (mc/q split)
    # doesn't leave stale files behind. mapping.json is overwritten below.
    for stale in exam_output_dir.glob("*.png"):
        stale.unlink()
    ...
    # Exam 1 is short-answer only (q01, q02, ...). Exam 2 Section A is
    # multiple-choice (mc01, mc02, ...) and Section B restarts numbering
    # for short answer (q01, q02, ...), each with its own counter.
    if paper.exam_type == "exam2":
        mcq_groups, short_groups = split_mcq_short_answer(question_groups)
        sections = [("mcq", mcq_groups), ("short", short_groups)]
        logger.info(
            "[%s] Split into %d MCQ + %d short-answer questions",
            paper.key,
            len(mcq_groups),
            len(short_groups),
        )
    else:
        sections = [("short", question_groups)]

    results: list[QuestionResult] = []
    counters = {"mcq": 0, "short": 0}
    for kind, groups in sections:
        prefix = "mc" if kind == "mcq" else "q"
        for group in groups:
            counters[kind] += 1
            parts = []
            pages_involved = []
            ...
            image_filename = f"{prefix}{counters[kind]:02d}.png"
            image_path = exam_output_dir / image_filename
            final_image.save(str(image_path))

            has_subquestions = paper.exam_type == "exam1" or kind == "short"
